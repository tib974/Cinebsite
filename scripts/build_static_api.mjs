...
function main() {
  ...
  if (fs.existsSync(realNdjson)) {
    ...
    // Also write JS module for bundling
    const jsModule = `// Auto-generated by build_static_api.mjs\nexport const STATIC_PRODUCTS = ${JSON.stringify(products, null, 2)};\nexport const STATIC_PACKS = ${JSON.stringify(packs, null, 2)};\nexport const STATIC_REALISATIONS = ${JSON.stringify(reals, null, 2)};\n`;
    fs.writeFileSync(path.join(root, 'src', 'static-data.mjs'), jsModule, 'utf8');
  } else {
